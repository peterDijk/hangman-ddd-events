...
export class EventStore implements IEventPublisher, IMessageSource {
  ...
  constructor(@Inject('EVENT_STORE_PROVIDER') eventStore: any) {
    this.category = 'games'; // QUESTION_PVD: this being hardcoded is not nice. what if there are more categories (/ domains? )
  ...
  async publish<T extends IEvent>(event: T) {
    ...
    const userId = message.userId || message.userDto.userId; // QUESTION_PVD: hardcoded stuff
    const streamName = `${this.category}-${userId}`; // QUESTION_PVD: hardcoded stuff
